...
def d_sparse_matrices(illumination_map: np.ndarray) -> csr_matrix:
  """Generates Toeplitz matrices of the compatible shape with the given ''illumination_map''
   ...
   ## Args:
        illumination_map (numpy.ndarray) : A shape-(M, N) array of maximum intensity values.

   ## Returns:
        d_x_sparse (scipy.sparse.csr_matrix) : A shape-(M*N, M*N) compressed sparse row matrix
        for calculation of a forward difference in a horizontal direction.

        d_y_sparse (scipy.sparse.csr_matrix) : A shape-(M*N, M*N) compressed sparse row matrix
        for calculation of a forward difference in a vertical direction.
  """
...
def partial_derivative_vectorized(input_matrix: np.ndarray, toeplitz_sparse_matrix: csr_matrix) -> np.ndarray:
  """Calculates a partial derivative of an ''input_matrix'' with a given ''toeplitz_sparse_matrix''.
   ...
   ## Args:
        ...
        toeplitz_sparse_matrix (scipy.sparse.csr_matrix) : A shape-(M*N, M*N) compressed sparse row matrix
        for calculation of a difference in a specified direction.

   ## Returns:
        p_derivative (numpy.ndarray) : A shape-(M, N) array of derivative values.
  """
  ...
  # Calculates values of partial derivatives with multiplication of the vectorized matrix by the specific 
  # Toeplitz matrix in a compressed sparse row format
  matrices_product = toeplitz_sparse_matrix * vectorized_matrix

  # Reverts vectorized matrix of partial derivatives to a shape of the input matrix
...
def gaussian_weight(grad: np.ndarray, size: int, sigma: Union[int, float], epsilon: float) -> np.ndarray:
  """Initializes weight matrix according to the third wieght strategy of the original LIME paper.
   ...
   ## Args:
        ...
        size (int) : An odd value which charactarizes the size of a Gaussian kernel.

        sigma (int or float) : A standard deviation value of a Gaussian kernel.

        epsilon (float) : A small value which prevents division by zero occurrences.
...
def initialize_weights(ill_map: np.ndarray, strategy_n: int, epsilon: float = 0.001) -> np.ndarray:
  ...
  and vectorizes these weight matrices preparing them to be used for calculation of a new illumination map. 
   ...
   ## Args:
        ill_map (numpy.ndarray) : A shape-(M, N) array of maximum intensity values.

        strategy_n (int) : A number of a selected strategy for weigth initialization. Could be 1, 2 or 3.

        epsilon (float) : A small value which prevents division by zero occurrences.

   ## Returns:
        flat_w_x (numpy.ndarray) : A shape-(1, M*N) vectorized array of updated weights with regard to horizontal direction.

        flat_w_y (numpy.ndarray) : A shape-(1, M*N) vectorized array of updated weights with regard to vertical direction.
  """
...
def update_illumination_map(ill_map: np.ndarray, weight_strategy: int = 3) -> np.ndarray:
  """Updates the initial illumination map according to a sped-up solver of the original LIME paper.
   ...
   ## Args:
        ...
        weight_strategy (int) : A number of a selected strategy for weigth initialization. Could be 1, 2 or 3.

   ## Returns:
        (numpy.ndarray) : A shape-(M, N) array of updated values of illumination map.
  """
  ...
  # Generates Toeplitz matrices of for computation of a forward difference in 
  ...
  # Updates the illumination map by solving the equation (19) of the original LIME paper
